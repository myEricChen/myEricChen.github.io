(function() {
        ...
        categories.forEach(cat => {
            ...
            if (cat.subcategories && cat.subcategories.length > 0) {
                ...
                cat.subcategories.forEach(sub => {
                    html += `<a href="/fr/products.html?category=${cat.id}&subcategory=${sub.id}" class="dropdown-sub-link">${sub.name}</a>`;
                });
                html += `</div>`;
            }
            html += `</div>`;
    ...
    if (!productId) {
        showError('Aucun produit spécifié. Veuillez sélectionner un produit dans la série d\'équipements.');
        return;
    }
    ...
    if (!product) {
        showError(`Produit avec l'ID "${productId}" introuvable.`);
        return;
    }
    ...
    document.getElementById('breadcrumb-category').href = `/fr/products.html?category=${category ? category.id : ''}`;
    ...
    function renderProductDetail(product) {
        ...
        // 处理徽章
        let badgesHtml = '';
        if (product.isNew) badgesHtml += '<span class="badge badge-new"><i class="fas fa-star"></i> Nouveau</span>';
        if (product.hasVideo) badgesHtml += '<span class="badge badge-video"><i class="fas fa-video"></i> Vidéo disponible</span>';
        if (product.hasManual) badgesHtml += '<span class="badge badge-manual"><i class="fas fa-file-pdf"></i> Manuel</span>';
        ...
        const html = `
            <div class="product-header">
                ...
                <div class="product-info-header">
                    ...
                    <div class="product-model">Modèle : ${product.model}</div>
    ...
    function showError(message) {
        ...
        container.innerHTML = `
            <div class="loading" style="color: #d71920;">
                ...
                <p>${message}</p>
                <a href="/products.html" class="btn btn-primary" style="margin-top: 20px;">Explorar serie de equipos</a>
            </div>
        `;
    }
})();